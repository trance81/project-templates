...
이때 문서 메타데이터(제목, 상태, 날짜), `pjt-docs/README.md` 목록, `CHANGELOG.md`도 함께 갱신됩니다.
...
`pjt-docs/`와 `.baton/`은 일반 Markdown 파일이므로 Git 없이도 사용할 수 있습니다. 다만 변경 이력을 자동으로 추적할 수 없습니다.

> [!TIP]
> 원격 저장소를 연결하지 않더라도 `git init`만 수행하는 것을 권장합니다.
> 이 구조에서는 문서의 변경 이력을 남기는 것이 중요하기 때문입니다.

Git을 사용할 수 없다면 다음을 보완합니다.

| 보완 항목 | 내용 |
|---|---|
| **변경 기록** | `pjt-docs/CHANGELOG.md`에 무엇을 왜 변경했는지 매번 기록 |
| **백업** | 프로젝트 폴더 전체를 정기적으로 백업하거나 동기화되는 위치에 보관 |
| **비공개 정보** | `.gitignore`가 동작하지 않으므로 `pjt-docs/local/`을 백업·공유 대상에서 직접 제외 |
