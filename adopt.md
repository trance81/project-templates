...
폴더 생성, 진입점 규칙 줄 추가, 훅 둘(세션 시작 안내 SessionStart, 배턴 없는 턴 종료를 막는
Stop) 등록, `.gitignore` 정리를 한 번에 처리한다.
...
- 템플릿에서 가져온 문서에 남아 있는 자리표시자를 실제 값으로 바꾼다. `YYYY-MM-DD`(문서 메타데이터의 `updated`와 README 지식 지도의 갱신일), `<프로젝트명>`, `<한 줄 소개>`, `<한 단락 요약>`이 그것이다. 자리표시자를 그대로 두면 그 문서는 언제 확인된 지식인지 알 수 없게 된다
- `.gitignore`에 `pjt-docs/local/` 추가 (`.baton/` 쪽은 4단계에서 스킬이 처리한다)
- `python scripts/check-docs.py`를 돌려 통과하는지 확인한다. 남은 자리표시자, 인덱스에 빠진 문서, 깨진 링크가 여기서 잡힌다. 문제가 있으면 고치고 다시 돌린다
- 커밋: `docs: pjt-docs 지식관리 구조 도입`

git 저장소가 아닌 프로젝트라면 원격 없이 `git init`만이라도 하도록 권한다. 사용자가 원치
않으면 강제하지 않되, `.baton/`의 Stop 훅이 동작하지 않아 세션 시작 안내만 남는다는 것과
`pjt-docs/local/`·`.baton/local/`을 백업·공유 대상에서 빼야 한다는 것을 알린다
(STRUCTURE.md 6장 "git 은 원격이 없더라도 쓴다").
